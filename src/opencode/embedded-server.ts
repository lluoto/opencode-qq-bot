...
function resolveOpencodeBin(): string {
  ...
  const candidates = [
    "C:\\nodejs\\opencode.cmd",
    join(process.env.APPDATA ?? join(home, "AppData", "Roaming"), "npm", "opencode.cmd"),
    join(home, ".bun", "bin", "opencode.exe"),
    join(home, ".bun", "bin", "opencode.cmd"),
  ]
  return candidates.find(existsSync) ?? "opencode.cmd"
}
...
function killProcessTree(pid: number): void {
  try {
    ...
    console.error(`[embedded-server] Failed to kill PID ${pid}; it may already have exited or require manual cleanup`)
  }
}
  ...
  if (alreadyRunning) {
    ...
    if (pid !== null) {
      ...
      return {
        ...
        forceClose: () => {
          console.log("[embedded-server] External server PID unavailable — cannot force shutdown")
        },
      }
    }
  }
