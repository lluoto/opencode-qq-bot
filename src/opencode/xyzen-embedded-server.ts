...
function loadEnvVars(): Record<string, string> {
  ...
  console.log("[embedded] Loaded API keys:", Object.keys(vars).filter(k => k.includes("KEY")))
  return vars
}
...
export class EmbeddedServer {
  ...
  async start(port = DEFAULT_PORT): Promise<{ url: string; close: () => void }> {
    ...
    return new Promise((resolve, reject) => {
      console.log("[embedded] 启动 OpenCode 服务器 on port", port)
      ...
      this.proc.stdout?.on("data", (chunk: Buffer) => {
        ...
        if (line.startsWith("opencode server listening")) {
          ...
          if (match) {
            ...
            this.isOwner = true
            resolved = true
            console.log("[embedded] 服务器已就绪 at", this.url)
      ...
      this.proc.on("exit", (code: number) => {
        if (!resolved) {
          console.error("[embedded] 进程退出 code", code)
