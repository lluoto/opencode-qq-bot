...
import { loadConfiguredModelIds } from "./configured-models.js"
...
export async function handleCommand(ctx: MessageContext, cmdCtx: CommandContext): Promise<string> {
  cmdCtx.pendingSelections.delete(deriveStateKey(ctx.botId, ctx.userId, ctx.groupId))
  ...
  switch (parsed.name) {
    ...
    case "mode":
      return handleMode(ctx, parsed.args, cmdCtx)
    case "plan":
      return handleMode(ctx, "plan", cmdCtx)
    case "build":
      return handleMode(ctx, "build", cmdCtx)
    case "rename":
    case "r":
...
export async function handlePendingSelection(
  ...
  const pending = cmdCtx.pendingSelections.get(stateKey)
  if (!pending) {
    return null
  }

  if (pending.expiresAt <= Date.now()) {
    cmdCtx.pendingSelections.delete(stateKey)
    return null
  }
  ...
  if (pending.type === "session") {
    cmdCtx.sessions.switchSession(ctx.userId, item.id, item.label)
...
async function handleStatus(ctx: MessageContext, cmdCtx: CommandContext): Promise<string> {
  ...
  return [
    ...
    `模型：${providerId && modelId ? `${providerId} / ${modelId}` : "跟随会话"}`,
    `Agent：${agentId ?? "默认"}`,
  ].join("\n")
}
...
async function handleSessions(ctx: MessageContext, cmdCtx: CommandContext): Promise<string> {
  ...
  cmdCtx.pendingSelections.set(stateKey, {
    type: "session",
...
export async function handleModel(ctx: MessageContext, args: string, cmdCtx: CommandContext): Promise<string> {
  const stateKey = deriveStateKey(ctx.botId, ctx.userId, ctx.groupId)
  if (!args) {
    const models = await listModels(cmdCtx.client, cmdCtx.configuredModelIds)
    if (models.length === 0) {
      return "当前没有可用模型"
    }
  ...
  if (/^\d+$/.test(args)) {
    const result = await handlePendingSelection(ctx, Number(args), cmdCtx)
  ...
  // Explicit model switch: check authorization against visible models
  const visibleModels = await listModels(cmdCtx.client, cmdCtx.configuredModelIds)
  const requestedModelId = `${model.providerId}/${model.modelId}`
  const isAuthorized = visibleModels.some((vm) => vm.id === requestedModelId)

  if (!isAuthorized) {
    return `模型 ${requestedModelId} 当前账号不可用`
  }
...
async function handleMode(ctx: MessageContext, args: string, cmdCtx: CommandContext): Promise<string> {
  const mode = args.trim().toLowerCase()
  if (mode !== "plan" && mode !== "build") {
    const current = cmdCtx.sessions.getAgent(ctx.userId) ?? "跟随会话"
    return [`当前模式：${current}`, "用法：/mode plan 或 /mode build"].join("\n")
  }

  await ensureSession(ctx.userId, cmdCtx)
  cmdCtx.sessions.setAgent(ctx.userId, mode)
  return `已切换模式：${mode}（重启后保留）`
}
...
async function listSessions(client: OpencodeClient): Promise<ListedSession[]> {
  ...
  return items
    .filter((item) => !extractProperty(item, "parentID") && !extractProperty(item, "parentId"))
    .map((item) => {
...
export async function listModels(client: ModelProviderClient, configuredModelIds?: ReadonlySet<string>): Promise<VisibleModel[]> {
  const response = await client.config.providers()
  return listVisibleModels(response, configuredModelIds ?? await loadConfiguredModelIds())
}
