...
let serverForceClose: (() => void) | null = null
let client: any = null
let router: EventRouter | null = null
let config: any = null
let gateway: any = null
let bridges: Array<ReturnType<typeof createBridge>> = []
let isShuttingDown = false
let consecutiveFailures = 0
let startPromise: Promise<boolean> | null = null
let restartInProgress = false
const PRIMARY_SERVER_PORT = 4096
const FALLBACK_SERVER_PORT = 4097

async function startServer(): Promise<boolean> {
  if (startPromise) return startPromise
  startPromise = startServerInternal()
  try {
...
async function startServerInternal(): Promise<boolean> {
  if (isShuttingDown) return false

  try {
    console.log("[index] 启动 OpenCode 服务器...")
    let server = await createOpencodeServer({ port: PRIMARY_SERVER_PORT })
    config.opencode.baseUrl = server.url
    serverClose = server.close
    serverForceClose = server.forceClose
    ...
    try {
      ...
      if (!server.owned) {
        console.error("[index] Reused OpenCode server is unhealthy; rebuilding it now")
        server.forceClose()
        server = await createOpencodeServer({ port: PRIMARY_SERVER_PORT })
        if (!server.owned) {
          console.error(`[index] Port ${PRIMARY_SERVER_PORT} is still held by an unhealthy server; using ${FALLBACK_SERVER_PORT}`)
          server = await createOpencodeServer({ port: FALLBACK_SERVER_PORT })
        }
        config.opencode.baseUrl = server.url
        serverClose = server.close
        serverForceClose = server.forceClose
    ...
    let bots = [{ appId: config.qq.appId, clientSecret: config.qq.clientSecret, sandbox: config.qq.sandbox }]
    try {
    ...
    for (const bot of bots) {
      ...
      startBackgroundTokenRefresh(bot.appId, bot.clientSecret)
      console.log(`[index] Bot ${bot.appId} 已启动 (模型：跟随会话)`)
    }
    ...
    if (serverForceClose) { serverForceClose(); serverForceClose = null }
...
async function restartServer(): Promise<void> {
  if (isShuttingDown || restartInProgress) return
